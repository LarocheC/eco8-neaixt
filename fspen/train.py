"""Training loop for FSPEN (ICASSP 2024) in this framework.

Uses the same plain-PyTorch convention as the other models here (CLI, resume,
g_<steps>/g_best checkpoints) wired to the shared VoiceBank-DEMAND dataset,
with the CMGAN recipe shared with ``lisennet/train.py``. FSPEN is end-to-end
(waveform in/out; STFT/iSTFT inside the model), so a batch is just
(clean, noisy) waveforms.

Losses (CMGAN, computed on power-compressed spectra — see fspen/model.py):
  generator   = w_com * (MSE(re) + MSE(im)) + w_mag * MSE(mag) + w_adv * adv
  discriminator (MetricGAN) = MSE(D(tgt,tgt), 1) + MSE(D(tgt,est), PESQ(tgt,est))
The CMGAN MetricDiscriminator is reused from common/discriminator.py. Unlike
LiSenNet there is no phase ambiguity: FSPEN predicts a complex mask, so the
enhanced phase is learned and no Griffin-Lim refinement exists anywhere.
"""
...
def train(a, h):
    ...
    model = build_fspen(h).to(device)
    discriminator = MetricDiscriminator(dim=int(h.get("disc_dim", 16))).to(device)
...
def main():
    print("Initializing FSPEN Training Process..")
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", default="configs/fspen.json")
    parser.add_argument("--checkpoint_path", default="cp_fspen")
