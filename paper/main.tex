...
\begin{figure*}[t]
  ...
  \caption{The NPU-hardened LiSenNet. The sub-band U-Net macro-structure is
  unchanged; the dual-path GRU bottleneck is re-expressed with NPU-mappable
  convolutions (blue; one block expanded below: a symmetric depthwise
  frequency convolution and a stack of causal dilated depthwise time
  convolutions with a convolutional GLU). Orange tags mark the four hardening
  swaps (Sec.~\ref{ssec:hardening}); violet chips mark the causal ring
  buffers that the streaming export exposes as explicit state I/O
  (Sec.~\ref{ssec:deploy}). Arrow labels give the frequency-axis size.}
  \label{fig:arch}
\end{figure*}
...
\begin{table*}[t]
  ...
  \begin{tabular}{@{}lrrrrcc@{}}
    \toprule
    deployment & weights & MAC/frame & ms/frame & RTF & block & PESQ \\
    \midrule
