...
\textbf{Deleting the FIFO is a trap --- and the numbers say why.} At the same
16\,ms latency, the stateless RF-window (block~b) is \textbf{11.6--26$\times$
slower} than streaming and misses real time for \emph{every} variant
(RTF 1.9--8.0). This is not because the window maps badly --- the opposite. It
uses the accelerator far \emph{better}: with no Slice/Concat state ops and a
69--197-frame convolution extent, it sustains 2.2--3.1\,GMAC/s against
streaming's 0.34--0.56, a $6\text{--}8\times$ higher utilization, confirming
that the $T{=}1$ streaming graph is launch- and state-bound rather than
compute-bound. But it must recompute the whole receptive field every frame:
$70\times$ the MACs at RF~68, $200\times$ at RF~196. A $6\text{--}8\times$
efficiency gain cannot pay a $70\text{--}200\times$ compute bill. Amortizing
over an emit block restores real time only by re-introducing latency
($T{\geq}3$, 48\,ms for 24\,ch; $T{\geq}9$, 144\,ms for relu6-deep) and even
then runs at 11--15\,ms/frame --- still $3\text{--}5\times$ costlier than
streaming, which needs no block at all. \emph{The FIFO graph dominates on both
axes}, which is what makes the \texttt{Pad} export fix of
Sec.~\ref{sec:background} the load-bearing step of this deployment: without
it, the only NPU-mappable graph was the window, and this model would not be
real-time on this chip. (Block~c is reported for completeness: at a 64-frame
block the window amortizes to 1.15\,ms/frame, but 1.02\,s of algorithmic
latency makes that a throughput mode, not a real-time one.)

\textbf{The FP32 \emph{decoder} is what fails to ship --- not ReLU6.} The
all-int8 relu6-deep graph deploys with only 6 software epochs, the same as
every other pure-int8 variant: ReLU6 folds into the quantization and adds
nothing to the epoch count. It is the \emph{hybrid} recipe --- the one that
leaves the decoder in FP32 for PESQ 3.052 --- that does not run: its float
decoder pulls 19 epochs onto the Cortex-M55 and needs 3.5\,MB of weights plus
12.3\,MB of activations, and it measures \textbf{34\,ms per frame, RTF 2.1}
($11\times$ the all-int8 windowed graph, past the budget). The same quality
knob without the float decoder --- Sec.~\ref{ssec:quant}'s activation clipping
--- recovers most of it (3.014) \emph{inside} the budget; that, and not the
FP32 decoder, is the deployable answer.
...
where a float decoder does not. Measuring both causal-context strategies at
equal latency also overturns an intuition: deleting the FIFO state buys a
$6\text{--}8\times$ better-utilized accelerator and still loses by
$12\text{--}26\times$, because recomputing a 68--196-frame receptive field
every frame is simply a bigger bill than the state plumbing it removes. The
deployed model streams frame-by-frame with 16\,ms algorithmic latency at
real-time factor 0.30 on a microcontroller NPU while \emph{exceeding} the
quality of the CPU-oriented original's deployable configuration by $+0.083$
PESQ. We believe the recipe
