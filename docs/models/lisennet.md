...
* **The stateless windowed graph is not available to it, by construction.** A GRU has
  ...
  finite RF can be recomputed statelessly. (Measured at equal latency the stateless
  window loses to FIFO streaming by 12–26× anyway — it recomputes the whole receptive
  field every frame — so in practice this costs the hybrid nothing.)
