...
**Withdrawn** (a number existed and was retracted, which is not the same as either of the above): a LiSenNet nc24 Cortex-M33 figure of ~21.2 M cycles/frame, a `dsp_offload` "13.12× speedup / 8080 µs" block, and a 9.8 KB dense arena figure. All three are listed in `deploy/rt595/results/PROVENANCE.md:59-68` as unsupported by any artifact. The 21.2 M figure is still quoted in prose at `deploy/rt595/BENCHMARKS.md:74-75`, and the 9.8 KB arena at `deploy/rt595/BENCHMARKS.md:98-99` — neither line should survive the reorganisation.
